...
-- Created by Administrator on 2020-12-18, 11:04:59 --
------------------------------------------------------

set define off
spool oascripts1218.log
...
BEGIN
  ...
  l_NcResult := l_response_call.extract('//return/text()').getStringVal();
   ...
   when others then
     errmsg := sqlerrm;
    insert into nc5.pm_oa_errlog(pk_billtype,pk_cont,oa_recid,message,message2) values (pk_billtype,pk_cont,iOaRecid,errmsg,errmsg);
   commit;
   raise_application_error(-20123,errmsg,true);
...
prompt ================================
prompt
create or replace function oa.fun_getpk_cont(cPk_corp varchar2,vPk_BillType varchar2,vContName varchar2,oabillno varchar2)
 return varchar2 is
  strResult varchar2(20);
begin

select max(pk_cont) into strResult from contno_oanc where oa_billno=oabillno;
...
prompt
prompt Creating function FUN_GETPK_PROJECT2
prompt ====================================
prompt
create or replace function oa.fun_getpk_project2(oabillno varchar2) return varchar2 is
  strResult varchar2(20);
begin

select max(pk_cont) into strResult from nc5.pm_cm_contract
 where pk_billtype in ('9241','9242','9243') and nvl(dr,0)=0 and vdef10=oabillno;

  return strResult;
end fun_getpk_project2;
/
...
prompt
prompt Creating function FUN_GETUSERDEPTINFO
prompt =====================================
prompt
create or replace function oa.fun_getuserdeptinfo(idCorp number,idDept number,idPerson number)
 return varchar2 is
  strResult varchar2(150);
str1 varchar2(50);
str2 varchar2(50);
str3 varchar2(50);
begin
strResult:='';
select max(name) into str1 from org_unit where id=idCorp;
select max(name) into str2 from org_unit where id=idDept;
select max(name) into str3 from org_member where id=idPerson;
...
strResult := strResult ||'->'||  str3;
    ...
    return   strResult;
end fun_getuserdeptinfo;
/

prompt
prompt Creating function SENDMSG
prompt =========================
prompt
create or replace function oa.sendMsg(pra varchar2)
return varchar2 as
LANGUAGE JAVA NAME 'MessageSender.sendMsg(java.lang.String) return java.lang.String';
/

prompt
prompt Creating procedure PF_ETL_CREATE_DIM_TIME
prompt =========================================
prompt
CREATE OR REPLACE PROCEDURE OA.PF_ETL_CREATE_DIM_TIME(beginYear in integer,
                                                    endYear in integer) is

  /*PF_ETL_CREATE_DIM_TIME: 生成时间维数据
...
BEGIN
  ...
  adddays := 1 ;
  dDate := to_date(to_char(beginYear*10000+101),'yyyymmdd');

  WHILE (dDate <= to_date(to_char(endYear*10000+1231),'yyyymmdd'))
  loop
     ...
     f_weekname :=f_yearname || to_char(dDate,'fmww')||'周';

     f_week   := f_year*100+ to_number(to_char(dDate,'fmww')); --201501
     f_weekday := to_char(dDate, 'day'); --星期几

     --dbms_output.put_line(f_weekday);
     ...
     */
     ...
     --dbms_output.put_line(f_monthname);

    insert into Dim_Time(FDATE,FYEAR,YEARNAME,FQUARTER, QUARTERNAME,FMONTH,MONTHNAME,dayname,Weekname,fWeek,Weekday
           ,Yearfirstdate,Yearlastdate,Quarterfirstdate,Quarterlastdate,Monthfirstdate,Monthlastdate,HBDayName)
                        values(dDate,f_year,f_yearname,f_quarter,f_quartername,f_month,f_monthname,f_datename,f_weekname,f_week,null
                             ,f_yearfirstday,f_yearlastday,f_quarterfirstday,f_quarterlastday,f_monthfirstday,f_monthlastday,f_nextDayName);

     dDate :=  dDate + adddays;
  END loop;

  update Dim_Time set Weekday =
                case trim(to_char(FDATE, 'day'))
...
prompt
prompt Creating procedure PROC_WRITELOG
prompt ================================
prompt
CREATE OR REPLACE PROCEDURE OA.
proc_writelog(sqlcode1 varchar2,sqlerrm2 varchar2)
is
PRAGMA AUTONOMOUS_TRANSACTION;
BEGIN
insert into oa_log
...
prompt
prompt Creating procedure SP_SAVERR
prompt ============================
prompt
create or replace procedure oa.sp_saverr(code varchar,name varchar) is
pragma autonomous_transaction;
begin
  insert into oa_log2(sqlcode,sqlerrm) values (code,name);
  commit;
end sp_saverr;
/
...
vNCResult clob;
begin
if iOaBillId is null then
return ;
end if; 

  if bRevoke = 1 then
    select max(1) into can_delete from nc5.pm_pa_payapply a where a.oa_recid=iOaBillId and nvl(dr,0)=0;
    if can_delete=1 then
      raise_application_error(-20001,'已有相关修订单记录不能删除');
      return;
    end if;
    update nc5.pm_pa_payapply a set a.dr=1 where a.oa_recid=iOaBillId ;
    return;
  end if;
...
select fun_getpk_cumanByPks(sPk_corp,pk_receiver_bas) into pk_receiver_man from dual;
  ...
  select FN_NCService('9261','',iOaBillId) into vNCResult from dual;
   insert into nc5.pm_oa_errlog(pk_billtype,pk_cont,oa_recid,message2,message) values ('9261','',iOaBillId,vNCResult,dbms_lob.substr(vNCResult,1000,1));
end;
/
...
--撤回审批
  if bRevoke = 1 then
    select max(1) into can_delete from nc5.pm_cm_contract a where  nvl(a.dr,0)=0 and a.vreserve20 = iOaBillId
    and pk_origcont is not null and pk_modiconttype is not null;
    if can_delete=1 then
      raise_application_error(-20001,'已有相关修订单记录不能删除');
      return;
    end if;
    update nc5.pm_cm_contract a set a.dr=1 where a.oa_recid=iOaBillId and pk_billtype=sNcBillType;
    return;
  end if;
...
select fun_getpk_cumanByPks(sPk_corp,pk_second_bas) into pk_second_man from dual;
  ...
  insert into nc5.pm_oa_errlog(pk_billtype,pk_cont,oa_recid,message2,message) values (sNcBillType,sPK_Cont,iOaBillId,vNCResult,dbms_lob.substr(vNCResult,2000,1));
  commit;
  insert into contno_oanc(oa_recid,oa_billno,pk_billtype,pk_cont,vbillno)
  select iOaBillId,t.vdef10,sNcBillType,t.pk_cont,t.vbillno from nc5.pm_cm_contract t where vreserve20=iOaBillId and t.pk_billtype in ('9241','9242','9243') and nvl(t.dr,0)=0
  and rownum=1
  order by t.pk_origcont desc,t.imodiver desc;
  commit;
...
return ;
  ...
  if bRevoke = 1 then
    select max(1) into can_delete from nc5.pm_pa_noncontfee a where nvl(a.dr,0)=0 and a.vreserve10 = iOaBillId;
    if can_delete=1 then
      raise_application_error(-20001,'已有相关修订单记录不能删除');
      commit;
      return;
    end if;
    update nc5.pm_pa_noncontfee a set a.dr=1 where a.vreserve10 = iOaBillId;
    commit;
    return;
  end if;
...
select fun_getpk_cumanByPks(sPk_corp,pk_receiver_bas) into pk_receiver_man from dual;
  ...
  insert into nc5.pm_oa_errlog(pk_billtype,pk_cont,oa_recid,message2,message) values ('9262','',iOaBillId,vNCResult,dbms_lob.substr(vNCResult,1000,1));

 commit;
   EXCEPTION
   when others then
      errmsg := sqlerrm;
    insert into nc5.pm_oa_errlog(pk_billtype,pk_cont,oa_recid,message2,message) values ('9262','',iOaBillId,errmsg,errmsg);
   commit;
...
vBillType_orig :='9241';
vBillType :='9247';--工程合同
select max(id) into iOaBillId from formmain_0208 where trim(field0001)=trim(:new.field0001);--根据合同名称查询ID
end if;
if :new.field0009 = 6778781750779463420 then
vBillType_orig :='9242';
vBillType :='9248';
select max(id) into iOaBillId from formmain_0205 where trim(field0001)=trim(:new.field0001);--根据合同名称查询ID
end if;
if :new.field0009 = 104504294407162084 then
vBillType_orig :='9243';
vBillType :='9249';
select max(id) into iOaBillId from formmain_0204 where trim(field0001)=trim(:new.field0001);--根据合同名称查询ID
end if;
sp_saverr('test',vBillType_orig||'---'||:new.field0001||'---'||iOaBillId);
--select max(pk_cont) into pk_cont1 from nc5.pm_cm_contract c ;

select max(icontstatus),max(nsumpaybasemny),max(pk_cont) into iContstat, nPayedmny,sPK_Cont 
from nc5.pm_cm_contract c where nvl(c.dr,0)=0 and trim(c.pk_billtype)=trim(vBillType_orig) 
and trim(c.vdef10)=trim(:new.field0001);

if sPK_Cont is null then
  sp_saverr(-20001,'NC系统中未查询到数据,'||vBillType_orig||'---'||:new.field0001||'---'||iOaBillId);
 -- raise_application_error(-20001,'NC系统中未查询到数据,'||vBillType_orig||'---'||:new.field0001||'---'||iOaBillId);
  return;
end if;

if nPayedmny > 0 then
  sp_saverr(-20001,'已有付款记录不可变更状态,'||vBillType_orig||'---'||:new.field0001||'---'||:new.field0008);
 -- raise_application_error(-20001,'已有付款记录不可变更状态,'||vBillType_orig||'---'||:new.field0001||'---'||:new.field0008);
  return;
end if;

if  :new.field0056 = 1253785730791001501 --and nvl(:old.field0056,0) <> 1253785730791001501
   then
  --作废
  update nc5.pm_cm_contract set dr=1,vbillstatus=0,icontstatus=0 where trim(vdef10)=trim(:new.field0001);
 return;  
end if;
...
if  :new.field0056 = 562560076970477410 and  nvl(:old.field0056,0) <> 562560076970477410 then 
--生效
 select FN_NCService(vBillType,sPK_Cont,iOaBillId) into vNCResult from dual;

 insert into nc5.pm_oa_errlog(pk_billtype,pk_cont,oa_recid,message2,message) values (vBillType,sPK_Cont,iOaBillId,vNCResult,dbms_lob.substr(vNCResult,3900,1));
end if;

exception
  when others then
 proc_writelog(sqlcode,sqlerrm);
 --raise_application_error(-20123,sqlerrm,true);
 end;
...
pk_zdr varchar2(20);
name1 varchar2(20);
message1 varchar2(250);
result1 varchar2(50);

messsage2 varchar2(150);
pk_jbr varchar2(20);
...
-- field0014 无合同付款
-- field0023
select fun_getpk_userByOaId(:new.field0023) into pk_zdr from dual;
if inserting and  pk_zdr is null then
select name into name1 from org_member where id=:new.field0023;
  message1:='用户'||name1||'在NC不存在 ，请在NC中添加。';
  result1:= sendMsg(message1);
end if;
...
select fun_getpk_psndocbyid(pk_corp1,:new.field0002) into pk_jbr from dual;
if inserting and pk_jbr is null then
  messsage2 :=fun_getuserdeptinfo(:new.field0004,:new.field0003,:new.field0002);
  if messsage2 is not null then
  message1:= messsage2||' 在NC不存在 ，请在NC中添加。';
  message1:= sendMsg(message1);
  end if;
end if;
...
select fun_getpk_cont(pk_corp1,'9242',:new.field0008,:new.field0029) into  pk_cont1 from dual;
...
--fun_getpk_projecttype  cProjectTypeName
 ...
 sp_savebillno(9245,:new.id,:new.field0001);
...
prompt
prompt Creating trigger TR_GCHT_XD
prompt ===========================
prompt
create or replace trigger oa.tr_gcht_xd
before insert or update
...
select fun_getpk_cont(pk_corp1,'9241',:new.field0008,:new.field0029) into  pk_cont1 from dual;
...
select fun_getpk_cubasdoc(:new.field0004,:new.field0015) into  pk_cubasdoc1st from dual;
select fun_getpk_cubasdoc(:new.field0004,:new.field0016) into  pk_cubasdoc2nd from dual;
select fun_getpk_project2(:new.field0010) into  pk_project from dual;
select fun_getpk_projecttype(:new.field0009) into  pk_conttype1 from dual;
...
select max(field0063) into vbillno1 from formmain_0209 where field0001=:new.field0010;
if vbillno1 is not null then
select max(pk_cont) into pk_cont1 from nc5.pm_cm_contract 
where nvl(dr,0)=0 and vname=:new.field0008 and icontstatus in (1,4) and pk_billtype in ('9241','9242','9243') and vbillno=vbillno1;
end if;

if pk_cont1 is null then
select max(pk_cont) into pk_cont1 from contno_oanc where oa_billno= :new.field0010;
end if;
...
select fun_getpk_cont(pk_corp1,'9243',:new.field0008,:new.field0029) into  pk_cont1 from dual;
...
-- field0014 无合同付款
-- field0023
select fun_getpk_userByOaId(:new.field0014) into pk_zdr from dual;
if inserting and pk_zdr is null then
select name into name1 from org_member where id=:new.field0014;
  message1:='用户'||name1||'在NC不存在 ，请在NC中添加。';
  result1:= sendMsg(message1);
end if;
...
select fun_getpk_psndocbyid(pk_corp1,:new.field0006) into pk_jbr from dual;--无合同
if inserting and pk_jbr is null then
  messsage2 :=fun_getuserdeptinfo(:new.field0015,:new.field0005,:new.field0006);
  if messsage2 is not null then
  message1:= messsage2||' 在NC不存在 ，请在NC中添加。';
  message1:= sendMsg(message1);
  end if;
